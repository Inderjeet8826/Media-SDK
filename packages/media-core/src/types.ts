...
export interface MediaEventMap {
  ...
  download: { item: MediaItem; format?: string; timestamp: number };
  search: { query: string; mediaType: 'photo' | 'video'; timestamp: number };
  error: { error: Error; context?: string; timestamp: number };
  [key: string]: unknown;
}
